    ...
    Boolean* outWillDoInPlace)
{
    ...
    // Passthru patch: also opt into the per-client ProcessOutput ('pout')
    // stage when mixing is enabled. Stock libASPL only negotiates
    // 'pmix'/'rite' under EnableMixing=true, so
    // IORequestHandler::OnProcessClientOutput() never fires and per-client
    // processing (per-app gain) is impossible without this. 'pout' does not
    // short-circuit later output ops (unlike 'mixo'), so the mixed-stream path
    // below is unchanged.
    //
    // A naive vendored upgrade that loses this block silently breaks
    // per-app gain. Keep this when refreshing third_party/libaspl.
    ...
    void* ioSecondaryBuffer)
{
    ...
    case kAudioServerPlugInIOOperationProcessOutput:
        // WF-009 patch: per-client pre-mix stage (see WillDoIOOperationImpl).
