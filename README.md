...
# Or: route system audio through Passthru at launch (the menu has the
# same toggle).
cd engine && swift run Passthru --route-through-passthru
```

Quit with the menu's Quit button or Ctrl+C. The previous default output
is restored on quit.
