...
let package = Package(
    ...
    targets: [
        ...
        // The host crash (NSInternalInconsistencyException via
        // SwiftUI's AppDelegate.makeMainMenu) was caused by @Published
        // writes from eventQueue; these tests pin the rule that all such
        // writes go through publishOnMain.
        .testTarget(
            name: "PassthruTests",
            dependencies: ["Passthru"],
            path: "Tests/PassthruTests"
        )
    ]
)
